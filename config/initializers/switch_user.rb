...
#   config.available_users = { user: -> { User.where.not("company_id IS NULL OR (first_name = ? AND last_name = ? AND admin = ?)", "Dan", "Lindblom", "FALSE") } }
...
#   config.redirect_path = ->(request, params) { '/user-profile' }
